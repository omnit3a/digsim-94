...
void e_behaviour_handle_screens (int * current_screen, int * frame_counter) {
  ...
  case GAMEOVER:
    if (IsKeyPressed(KEY_ENTER)){
      e_storage_save_value(STORAGE_POSITION_HIGH_SCORE, player_get_info().score);
  ...
  case MENU:
    new_current_screen = e_behaviour_handle_menu(new_current_screen);
    break;
...
int e_behaviour_handle_menu (int current_screen) {
  ...
  if (IsKeyPressed(KEY_ESCAPE)) {
    new_current_screen = GAMEPLAY;
    return new_current_screen;
  }
  
  if (IsKeyPressed(KEY_Q)) {
    SetExitKey(KEY_ESCAPE);
    int high_score = e_storage_load_value(STORAGE_POSITION_HIGH_SCORE);
    if (new_player_info.score > high_score) {
      e_storage_save_value(STORAGE_POSITION_HIGH_SCORE, new_player_info.score);
      e_render_update_score();
    }
